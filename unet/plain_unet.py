...
class PlainUnet(object):
    def __init__(self):
        ...
        self.block_nums = self.config.get('block_nums')
        self.training = True

    def build_blocks(self, inputs, out_filters, block_num):
        if block_num < 1:
            raise ValueError('block_num must be >= 1')
    ...
    def forward(self, inputs):
        ...
        for i, filters in enumerate(self.block_filters):
            outputs = self.build_blocks(outputs, filters, self.block_nums[i])
